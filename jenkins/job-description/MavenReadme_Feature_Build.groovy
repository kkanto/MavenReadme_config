...
def job_config = [
    job: [
        name: "MavenReadme_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'MavenReadme_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/MavenReadme_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.MavenReadme.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "kkanto", "MavenReadme", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("MavenReadme_Feature_Build");
...
job.with {
    ...
    authenticationToken('N5P2gShqA654KtdAI1ZQWWFkLJwRqLzB')
